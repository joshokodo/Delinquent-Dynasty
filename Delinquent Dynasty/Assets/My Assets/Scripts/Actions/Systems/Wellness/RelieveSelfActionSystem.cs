...
[BurstCompile]
public partial struct RelieveSelfActionSystem : ISystem {
    private PerformActionUtil _perform;
    private TriggerActionEffectsUtil _triggerEffects;

    public void OnCreate(ref SystemState state){
        state.RequireForUpdate<StomachActionFunction>();
        _perform.SetUp<StomachActionFunction>(ref state, new DynamicActionType(MiscActionType.RELIEVE_SELF), 0);
        _triggerEffects.SetUp<StomachActionFunction>(ref state, new DynamicActionType(MiscActionType.RELIEVE_SELF), 1);
    }
