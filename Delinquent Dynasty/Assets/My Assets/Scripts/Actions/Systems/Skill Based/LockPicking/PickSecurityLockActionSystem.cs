...
[BurstCompile]
public partial struct PickSecurityLockActionSystem : ISystem {
    ...
    public void OnCreate(ref SystemState state){
        _actionType = new DynamicActionType(SkillBasedItemActionType.PICK_SECURITY_LOCK);
        ...
        _mainPhaseQuery =
            state.GetEntityQuery(
                CommonSystemUtils.BuildCommonActionTargetsComponentConditionsFunctionPerformQuery<ArmsMentalActionFunction>());
    ...
    public void OnUpdate(ref SystemState state){
        ...
        var h2 = new PickSecurityLockMainThreadJob(){
            DynamicActionType = _actionType,
            Ecb = ecb,
...
[BurstCompile]
public partial struct PickSecurityLockMainThreadJob : IJobEntity {
    ...
    public void Execute(Entity e, CharacterBehaviorEntityComponent comp, DynamicBuffer<ActiveActionElement> actions,
        ...
        if (actionUtils.TryGetActiveActionAndTargets(DynamicActionType, actions, targets,
            ...
            // TODO: add success chance here
            if (SecurityLockLookup.TryGetComponent(targetLock, out ItemSecurityLockComponent lockComponent)){
