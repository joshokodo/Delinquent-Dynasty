...
public struct XIsRecentFightingActionTargetOfYConditionLogic : IConditionCheck {
    public bool Result(ConditionUtils utils, ConditionData conditionData){
        ...
        var actionOrigin = utils.TargetsGroup.GetSecondaryTargetEntity(conditionData);
        ...
        foreach (var visibleCharacterElement in visibility){
            if (visibleCharacterElement.VisibleCharacter != actionOrigin) continue;
            ...
            foreach (var act in actions){
                if (act.ActionType.IsFightingAction){
                ...
                foreach (var knowledgeElement in knowledge){
                    ...
                    foreach (var eventKnowledgeElement in events){
                        if (eventKnowledgeElement.PerformingEntity == visibleCharacterElement.VisibleCharacter
                            && eventKnowledgeElement.ActionType.IsFightingAction
                            && eventKnowledgeElement.ContainsTarget(target)
                            && eventKnowledgeElement.ActionTimestamp.TimeStamp.MatchesSeasonAndYear(utils.InGameTime)){
                            var difference = utils.InGameTime.TotalInGameSeconds -
                                             eventKnowledgeElement.ActionTimestamp.TimeStamp.TotalInGameSeconds;
